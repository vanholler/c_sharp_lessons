...
public class UpdateHandler : IUpdateHandler
{
    ...
    public void HandleUpdateAsync(ITelegramBotClient botClient, Update update)
    {
        ...
        try
        {
            ...
            switch (command)
            {
                ...
                case "/find":
                    HandleFind(botClient, chat, user!, text);
                    break;
    ...
    private void HandleShowTasks(ITelegramBotClient botClient, Chat chat, ToDoUser user)
    {
        ...
        PrintTasks(botClient, chat, tasks, "Активных задач нет.");
    }
    ...
    private void HandleFind(ITelegramBotClient botClient, Chat chat, ToDoUser user, string input)
    {
        string? namePrefix = GetCommandArgument(input);
        if (string.IsNullOrWhiteSpace(namePrefix))
        {
            botClient.SendMessage(chat, "Используйте: /find <префикс>");
            return;
        }

        var tasks = _toDoService.Find(user, namePrefix.Trim());
        PrintTasks(botClient, chat, tasks, "Задачи не найдены.");
    }
        ...
        string emptyMessage)
    {
        if (tasks.Count == 0)
        {
            botClient.SendMessage(chat, emptyMessage);
            return;
        }
